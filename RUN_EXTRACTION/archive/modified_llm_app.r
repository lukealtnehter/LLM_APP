...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      # Preview sections moved to main panel
      fluidRow(
        column(6,
               div(
                 h5(strong("Current Prompt"), style = "color: #337ab7;"),
                 div(id = "prompt_preview", class = "preview-box",
        ...
        column(6,
               div(
                 ...
                 div(id = "schema_preview", class = "preview-box",
                     verbatimTextOutput("schema_text"))
               )
        )
      ),
...
server <- function(session, input, output) {
  ...
  output$show_prompt_preview <- reactive({
    !is.null(values$prompt_content)
  })
  
  output$show_schema_preview <- reactive({
    !is.null(values$schema_content)
  })
  ...
  outputOptions(output, "show_schema_preview", suspendWhenHidden = FALSE)
  outputOptions(output, "show_data_table", suspendWhenHidden = FALSE)
  
  # Preview outputs
  output$prompt_text <- renderText({
    if(!is.null(values$prompt_content)) {
      values$prompt_content
    }
  })
  
  output$schema_text <- renderText({
    if(!is.null(values$schema_content)) {
